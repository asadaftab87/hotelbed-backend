...
async function importTestHotelsOnly(connection) {
  console.log('📥 Importing test hotel data only...\n');
  ...
  for await (const line of invRl) {
    ...
    if (TEST_HOTELS.includes(hotelId)) {
      ...
      invBatch.push(values);
      
      if (invBatch.length >= 1000) {
        const placeholders = invBatch.map(() => '(?,?,?,?,?,?)').join(',');
        await connection.query(
          `INSERT INTO hotel_inventory (hotel_id,room_code,board_code,date_from,date_to,availability_data) VALUES ${placeholders}`,
          invBatch.flat()
        );
  ...
  for await (const line of rateRl) {
    ...
    if (TEST_HOTELS.includes(hotelId)) {
      ...
      rateBatch.push(values);
      
      if (rateBatch.length >= 1000) {
        const placeholders = rateBatch.map(() => '(?,?,?,?,?,?,?,?,?,?,?)').join(',');
        await connection.query(
          `INSERT INTO hotel_rates (hotel_id,room_code,board_code,date_from,date_to,rate_type,base_price,tax_amount,adults,board_type,price) VALUES ${placeholders}`,
          rateBatch.flat()
        );
...
async function showPricing(connection) {
  ...
  for (const hotel of hotels) {
    ...
    console.log(`   Total rates: ${rates[0].count}`);
    ...
    if (rates[0].count > 0) {
      ...
      samples.forEach(r => {
        console.log(`      ${r.room_code}/${r.board_code} | ${r.adults}p | €${r.price} | ${r.date_from}-${r.date_to}`);
      });
    }
  }
}
...
async function main() {
  console.log('🚀 Import Test Hotels Only\n');
  
  const connection = await mysql.createConnection(DB_CONFIG);
  
  await importTestHotelsOnly(connection);
